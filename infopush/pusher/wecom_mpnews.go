...
func getWecomAccessToken(config WecomMPNewsConfig) (string, error) {
	...
	response, err := common.HttpRequest("GET", url, nil, 30*time.Second)
	if err != nil {
		return "", err
	}
...
func SendWecomMPNews(configData map[string]any, params map[string]string) (string, error) {
	// 转换配置
	...
	response, err := common.HttpRequest("POST", url, jsonData, 30*time.Second)
	if err != nil {
		return "", err
	}

	responseStr := string(response)
	return common.HandleAPIResponse(responseStr, `"errcode":0`)
}

// convertToWecomMPNewsConfig 将通用配置转换为企业微信配置
func convertToWecomMPNewsConfig(config map[string]any) (WecomMPNewsConfig, error) {
	// 使用类型断言提取配置值
