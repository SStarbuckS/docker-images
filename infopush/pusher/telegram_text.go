...
func SendTelegramText(configData map[string]any, params map[string]string) (string, error) {
	// 转换配置
	...
	return common.HandleAPIResponse(responseStr, `"ok":true`)
}

// convertToTelegramTextConfig 将通用配置转换为Telegram文本配置
func convertToTelegramTextConfig(config map[string]any) (TelegramTextConfig, error) {
	// 使用类型断言提取配置值
