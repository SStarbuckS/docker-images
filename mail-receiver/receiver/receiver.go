...
func (ar *AccountReceiver) run() error {
	...
	// 首先处理现有邮件
	if err := ar.fetchAndProcessMessages(folder); err != nil {
		return err
	}
	...
	for {
		...
		// 等待监控结果
		event, ok := <-monitor.UpdateCh
		if !ok {
			return fmt.Errorf("监控通道已关闭")
		}
		if event.Err != nil {
			return event.Err
		}
...
func (ar *AccountReceiver) fetchAndProcessMessages(folder string) error {
	...
	for _, msg := range messages {
		...
		if ar.pusher != nil {
			...
			if err != nil {
				...
				// 推送成功，标记邮件为已读
				if err := ar.client.MarkAsRead(email.UID); err != nil {
					log.Printf("[%s] 已推送但标记已读失败 (UID: %d, 主题: %s): %v", ar.name, email.UID, email.Subject, err)
				} else {
					log.Printf("[%s] 已推送并标记已读: %s", ar.name, email.Subject)
				}
			}
		}
