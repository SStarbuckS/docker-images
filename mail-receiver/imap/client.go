...
type MonitorResult struct {
	UpdateCh <-chan MonitorEvent // 更新通知通道
}

// MonitorEvent 表示一次监控周期的结果。
...
func (c *Client) Login() error {
	...
	supportsIDLE, err := c.idleClient.CheckIDLESupport()
	if err != nil {
		return err
	}
	c.supportsIDLE = supportsIDLE
	if !supportsIDLE {
		log.Printf("[%s] 服务器不支持 IDLE，使用轮询模式", c.accountName)
	}
...
func (c *Client) IdleWithFallback(folder string, pollInterval time.Duration) *MonitorResult {
	updateCh := make(chan MonitorEvent)
...
// idleMode IDLE模式监听
func (c *Client) idleMode(folder string, updateCh chan<- MonitorEvent) {
	log.Printf("[%s] 使用 IDLE 模式监控文件夹: %s", c.accountName, folder)

	// 使用IDLE客户端监控
	idleUpdateCh := c.idleClient.MonitorWithIDLE(folder)

	result, ok := <-idleUpdateCh
	if !ok {
		// IDLE监控结束（idle.go中已输出详细日志）
		updateCh <- MonitorEvent{Err: fmt.Errorf("IDLE 已结束")}
		return
	}
	if result.Err != nil {
		updateCh <- MonitorEvent{Err: result.Err}
		return
	}
	if result.HasUpdate {
		// 有新邮件更新
		log.Printf("[%s] IDLE 收到新邮件通知", c.accountName)
		updateCh <- MonitorEvent{HasUpdate: true}
		return
	}
	updateCh <- MonitorEvent{}
}

// pollMode 轮询模式
func (c *Client) pollMode(folder string, interval time.Duration, updateCh chan<- MonitorEvent) {
